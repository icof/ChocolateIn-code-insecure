...
2. Adaptez la valeur du serveur MySQL auquel on effectue la connexion dans la variable `$_ENV["host"]` des fichiers `site/back/modele/bd.inc.php` et `site/front/modele/class.pdochoc.inc.php`. Sur un serveur local, elle est classiquement à "localhost".
